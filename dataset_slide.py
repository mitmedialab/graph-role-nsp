...
class SpeedDatingDS(Dataset):
    def __init__(self, group_id, social_rel, context_len = 5, hop = 1, dir_path = "./SpeedDating/", transform=None):
        ...
        self.nvb_df = self.nvb_df.sort_values(by = ['End_frame'])
    ...
    def __len__(self):
        return int((self.vb_df["End_frame"].tail(1).item() - self.context_len)//(self.fps*self.hop))

        
    def __getitem__(self, idx):
        
        speaking_time = self.context_len + idx*self.fps
        
        curr_context_nvb = self.nvb_df[self.nvb_df['Start_frame'].le(speaking_time)]
        curr_context_nvb = curr_context_nvb[curr_context_nvb['End_frame'].ge(speaking_time - self.context_len)]

        curr_context_vb = self.vb_df[self.vb_df['Start_frame'].le(speaking_time)]
        curr_context_vb = curr_context_vb[curr_context_vb['End_frame'].ge(speaking_time - self.context_len)]
        ...
        for i, speaker in enumerate(self.unique_person):
            ...
            if not speaker_series.empty:
                ...
                speaker = speaker_series.tail(1)['Participant'].item() 
                i = int(np.where(self.unique_person == speaker_series.tail(1)['Participant'].item())[0])
...
# loader = DataLoader(SD, batch_size = 1, shuffle = True)
